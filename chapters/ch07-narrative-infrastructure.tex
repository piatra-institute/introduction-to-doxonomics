...
The narrative ``deregulation promotes efficiency'' has extensive infrastructure on both sides of the Atlantic: in the US, well-funded think tanks (Heritage, Cato, AEI) connected to prestigious op-ed pages (\textit{WSJ}, \textit{FT}), cable news panels, congressional testimony channels, and economics curricula; in the UK and across Europe, the Institute of Economic Affairs (IEA), the Adam Smith Institute, and their European-network partners feed the same pipeline of analyst quotes, parliamentary testimony, and policy templates \autocite{cockett1994, plehwe2006}.
The maximum pathway efficiency to policymakers in either jurisdiction is high.

The narrative ``public ownership can be efficient'' has minimal infrastructure: a few academic advocates, limited think-tank support, almost no connections to major op-ed pages or broadcast panels.
...
Constructing a narrative pipeline is a generalizable doxonomic project: it requires sustained investment over decades, coordinated philanthropy or party financing, and an intellectual seed network that can be cashed in later for op-eds, parliamentary testimony, and curricular standing.
Three documented build-outs illustrate the pattern.

\paragraph{Transnational seed (Mont P\`elerin Society, 1947--).}
A small network of European and US intellectuals founded the Mont P\`elerin Society to recover and modernize classical liberalism after the Second World War \autocite{mirowski2009, plehwe2006}.
The Society had no policy programme; it produced the credentialed cadres later deployed by national-level institutions.
This is the model of the \emph{intellectual seed network}: small, prestigious, transnational, and patient.

\paragraph{The British build-out (IEA / Adam Smith Institute, 1955--1985).}
The Institute of Economic Affairs (founded 1955) and the Adam Smith Institute (founded 1977) cultivated the policy ground for the Thatcher government \autocite{cockett1994, stedmanjones2012}.
By 1979 the IEA's privatization and monetarist templates were in circulation through Conservative MPs, sympathetic press (chiefly the \textit{Times} under Rees-Mogg, the \textit{Telegraph}, and the \textit{Sunday Times}), and senior civil servants.
The same pipeline became the source-side template for cross-Atlantic transfer.

\paragraph{The US build-out (1947--2020).}
The US conservative infrastructure was consciously constructed over four decades \autocite{mayer2016, phillipsfein2009}:
\begin{itemize}[nosep]
  \item \textbf{1947}: Mont P\`{e}lerin Society founded (transnational seed; see above).
...
\paragraph{Replication at planetary scale (Atlas Network, 1981--).}
The Atlas Network, founded in 1981, has since seeded or supported more than 500 affiliated free-market institutes across roughly 95 countries, exporting the IEA/Heritage template to jurisdictions from Latin America to South and Southeast Asia and Eastern Europe \autocite{djelic2014, plehwe2006}.
The pattern is now demonstrably portable: the same playbook, with local funders and language partners, produces structurally comparable infrastructure outside its original Anglo-American setting.

No comparable centre-left infrastructure was built during the same period in any of these jurisdictions \autocite{mudge2018}.
The infrastructure asymmetry, not a difference in the quality of ideas, explains much of the rightward shift in policy common sense across the OECD between 1980 and 2020.
\end{example}
