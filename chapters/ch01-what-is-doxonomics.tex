...
This is the condition that Bourdieu called \textit{doxa}: a belief so deeply embedded that it is no longer experienced as a belief at all \autocite[pp.~164--170]{bourdieu1977}.
...
The belief ``markets are the most efficient allocators of resources'' can be traced through a concrete chain, parallel pipelines having been built on both sides of the Atlantic and replicated globally:
\begin{enumerate}[nosep]
  \item \textbf{Funding}: in the US, Koch, Scaife, Olin, and Bradley foundations directed hundreds of millions of dollars into free-market intellectual infrastructure from the 1970s onward \autocite{mayer2016}; in the UK and across Europe, comparable seed money flowed through the Institute of Economic Affairs and the Mont P\`elerin transnational network \autocite{cockett1994, plehwe2006}.
  \item \textbf{Institutional processing}: Heritage Foundation, Cato Institute, American Enterprise Institute, and Mont P\`{e}lerin Society in the US; the IEA, Adam Smith Institute, and Centre for Policy Studies in the UK; and 500+ Atlas Network affiliates across roughly 95 countries produced research, policy briefs, and expert testimony \autocite{mirowski2009, djelic2014}.
  \item \textbf{Narrative output}: op-eds, books (\textit{Free to Choose}, \textit{The Road to Serfdom}), textbooks (Mankiw's \textit{Principles of Economics}; locally-adapted equivalents in EU, Latin American, and South Asian curricula), and television programs.
  \item \textbf{Circulation}: editorial pages of the \textit{Wall Street Journal}, \textit{Financial Times}, \textit{The Economist}; economics curricula at thousands of universities worldwide; cable news; social media.
  ...
  \item \textbf{Behavioral consequence}: deregulation, privatization, austerity, reduced union membership, increased tolerance for inequality (the trajectory holds across the OECD).
...
\end{enumerate}
This is documented institutional history with named actors, traceable funding, and measurable outcomes, not a conspiracy theory.
The doxonomic contribution is to treat it as a \emph{system} with formal structure, rather than as a sequence of political events.
\end{example}
