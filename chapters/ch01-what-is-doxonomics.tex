...
That is an empirical question---and the experimental evidence is considerably more nuanced than the institutional consensus suggests (Chapter~\ref{ch:human-nature}).
But the doxonomic question is different: \emph{what institutional apparatus sustains this belief, at what cost, through which channels, and with what social effects?}
...
The purpose of the tuple is to establish that doxonomics has a \emph{formal object}---not just a topic of conversation, but a mathematical structure that can be analyzed, compared, and measured.
\end{remark}
...
When the chain closes---when the beliefs produced by the system generate exactly the behaviors and institutions that sustain those beliefs---the system reaches a \emph{doxonomic equilibrium}.
...
Doxonomics studies the \emph{differential amplification} of beliefs---the fact that some beliefs receive vastly more institutional support than others---not the claim that evidence is irrelevant.
...
A doxonomic system that spends billions may still fail to achieve capture---and understanding why it fails is as important as understanding why it succeeds.
...
On the media side, \textcite{lippmann1922} pioneered the analysis of how media manufactures public opinion.
...
Identify a doxonomic system that operates primarily through non-monetary channels---for example, family socialization, religious ritual, or architectural design.
