...
But new cracks had appeared: the Occupy movement (2011, multi-city US and globally), the Indignados in Spain and the Greek anti-austerity movement (2011--2015), the Yellow Vests in France (2018--), the Bolsonaro and Lula re-mobilizations in Brazil, and the Sanders and Trump campaigns in the US (2016) all reflected persistent legitimation failure across multiple democracies; the ``rigged system'' discourse and its EU and Latin American cognates indexed the same crack from different political angles.
...
\subsection{Temporal succession: from production to valuation}
\label{sec:ch24-temporal-succession}

Cross-country comparison is one cut; cross-period comparison is the other.
Legitimation regimes are not historically fixed. The dominant narrative repertoire shifts when the underlying source of elite wealth shifts, and the shift is itself a doxonomic event: each new regime reorganizes the relationship between wealth, value, and the work the narrative apparatus is asked to do.

\begin{center}
\footnotesize
\begin{tabular}{p{2.1cm}p{2.4cm}p{2.4cm}p{2.4cm}p{2.4cm}}
\toprule
 & \textbf{Productive capitalism} & \textbf{Financial capitalism} & \textbf{Avarism} & \textbf{Valuationism} \\
 & ($\sim$1750--1930) & ($\sim$1930--1980) & ($\sim$1980--2010) & ($\sim$2010--present) \\
\midrule
\textbf{Wealth object} & ownership of productive assets & claims on future profits (securities, derivatives) & accumulation itself, treated as virtue & narratively-maintained future claims; collateralizable expectation \\[6pt]
\textbf{Source of power} & profit extraction from labor and resources & rents from financial intermediation & cultural permission for accumulation; meritocratic warrant & narrative control over future valuation; access to collateral \\[6pt]
\textbf{Legitimating narrative} & creation of useful goods, employment, national output & efficient allocation, market discipline, capital as engine of growth & wealth as evidence of merit; the entrepreneur as moral exemplar & technological inevitability; future-monopoly story; founder as visionary \\[6pt]
\textbf{Ruling figure} & industrialist, factory owner & financier, hedge-fund manager & CEO-celebrity, dynastic billionaire & founder-prophet, narrative-engine principal \\[6pt]
\textbf{Doxonomic load} & moderate (production speaks for itself) & high (markets must be presented as efficient) & high (excess must be presented as desert) & very high (valuation must be continuously performed) \\[6pt]
\textbf{Failure mode} & overproduction, labor revolt & financial crisis, mass insolvency & legitimation crisis (post-2008) & narrative collapse, collateral run \\
\bottomrule
\end{tabular}
\end{center}

The regimes do not displace one another. They overlay. Productive capitalism still exists; financial capitalism still operates; the avaristic moral horizon, in the user-supplied sense of accumulation-as-virtue \autocite{sandel2012, veblen1899}, still organizes much of contemporary elite self-presentation. What changes is the \emph{marginal} source of power, and therefore the \emph{marginal} demand on the doxonomic apparatus.

The valuationist regime is the case where the doxonomic load on the legitimation apparatus is maximal. The ruling figure's nominal wealth is mostly mark-to-market equity in firms whose share prices depend on a continuously maintained narrative about future monopoly outcomes. The wealth is not cash, and not even claims on near-term cash flow; it is the present-discounted value of a story about the future, sustained by a multi-channel network of friendly analysts, founder-controlled communications, sympathetic regulators, and a fan base whose attention is itself part of the collateral structure. Soros's notion of \emph{reflexivity} \autocite{soros1987} describes the dynamic in financial markets; valuationism is reflexivity industrialized into a regime of accumulation \autocite{krippner2011, tooze2018}.

The chapters that already carry the formal apparatus for this analysis are \ref{ch:narrative-infrastructure} (the narrative pipeline that maintains the valuation; the analyst-influencer-media layer is structurally analogous to the policy-think-tank layer), \ref{ch:common-sense-capture} (the captured proposition is the share price itself; once captured, dissent reads as category error rather than as a position), \ref{ch:doxonomic-power} (the conversion of belief into command), and \ref{ch:technology-inevitability} (the future-monopoly stories that supply the mythic object).
The first-order belief equation (Model~1.1) applies directly: replace $B_C$ (a public belief) with $P_C$ (a share price or private valuation), and the structural form is the same.
Proposition~\ref{prop:resource-asymmetry}'s prediction --- that belief tracks funding ratios when production is concentrated --- extends naturally: in valuationism, asset prices track narrative-production ratios when the firm's fundamentals are too far in the future to discipline the price.

The interesting case is what happens when the regimes fall out of phase. The 2008 financial crisis exposed the gap between financial capitalism and the productive economy and produced a legitimation crisis (§\ref{sec:ch24-failure}). A comparable exposure is structurally possible in the valuationist layer: a forced realization (margin call, regulatory action, technological disappointment) that revealed the gap between mark-to-market valuation and underlying productive capacity. The narrative apparatus would then face the same problem the financial-capitalist apparatus faced after 2008, but at a layer further removed from the productive base.
...
On the racial dimensions of inequality legitimation, see \textcite{du-bois2007} on the wages of whiteness and \textcite{hochschild2016} on the American Dream's racial contours; for parallel work outside the US, see Souza (2017) on Brazilian inequality legitimation, Bhattacharya (2018) on caste-class compounding in India, and Seekings and Nattrass (2005) on post-apartheid inequality discourse in South Africa.
