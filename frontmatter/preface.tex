...
\textbf{Doxonomics} is the science of how material power shapes public belief.
The term combines the Greek \textit{doxa} (opinion, common belief, public orthodoxy) with \textit{-nomics} (the systematic study of allocation and structure).
...
The field needs stock-and-flow models, causal inference, network analysis, information-theoretic measures, and sheaf-theoretic coherence conditions where the structure demands them.
...
A note on the formal environments used throughout the book:
\emph{Theorems} are results that follow rigorously from stated assumptions (they may be conditional on stylized premises, but the derivation is exact).
\emph{Propositions} are model-derived claims: they follow from a formal model's assumptions but depend on those assumptions being a reasonable approximation of reality, which is often an open empirical question.
\emph{Models} are explicit simplifications of complex systems, useful for generating predictions and structuring analysis but not to be confused with descriptions of reality.
\emph{Conjectures} are claims we believe to be true but cannot yet prove or have not yet tested.
\emph{Definitions} fix terminology; they are stipulative, not empirical.
\emph{Remarks} are observations, interpretive claims, or conceptual points that do not have the formal status of derived results.
Where illustrative numerical examples appear, they are order-of-magnitude estimates or hypothetical scenarios designed to show the structure of an argument, not empirically calibrated measurements, unless a specific source is cited.
The reader should calibrate their confidence accordingly: the architecture is more mature than the measurement, and the measurement is more mature than the prediction.
...
The reader is invited to learn doxonomics and to extend it.
